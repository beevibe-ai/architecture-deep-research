...
## Status: all 13 items shipped

| # | Item | Commit |
| --- | --- | --- |
| 1 | Cite-rot check + health snapshot | `a32d261` |
| 2 | `adr principles refresh` | `2d3ece2` |
| 3 | Accept/edit/skip stats persistence | `b4b5af5` |
| 4 | Suppression syntax `// adr-ignore: <id>` | `52f19d2` (+ `1c47b7d` wildcard bug fix) |
| 5 | Confidence auto-evolves from stats | `46bd48a` |
| 6 | `adr principles incremental` | `aa7ec71` |
| 7 | `adr principles refine --id <id>` | `f73c051` |
| 8 | Quality tuning (exact lines, confidence rule, antipattern push) | `fd8bc5b` |
| 9 | `adr drift` — full-repo scan vs principles | `9b3c2c3` |
| 10 | GitLab + Bitbucket posters | `2039781` |
| 11 | `adr guard uninstall` | `2dfd5ed` |
| 12 | Batched CI review mode (`--batch`) | `da909ab` |
| 13 | Test coverage + diff-parser edge cases | `1c47b7d` |

The README's "in development" list — `adr drift`, the rest of the AI CTO loop — is now empty except for **the brain (Phase 4)**.

---

## Original priority list (with status)

### Evolvability — preventing the static-lint trap

#### 1. Cite-rot check + `health` block in principles.json — SHIPPED `a32d261`
...
#### 2. `adr principles refresh` — incremental refresh — SHIPPED `2d3ece2`
...
#### 3. Accept/edit/skip stats persisted per principle — SHIPPED `b4b5af5`
...
#### 4. Suppression syntax `// adr-ignore: <principle-id>` — SHIPPED `52f19d2` (+ `1c47b7d`)
...
#### 5. Confidence auto-evolves from review feedback — SHIPPED `46bd48a`
...
#### 6. `adr principles incremental` — runs on changed files since last refresh — SHIPPED `aa7ec71`
...
#### 7. `adr principles refine <id>` — single-principle re-discovery — SHIPPED `f73c051`
...
#### 8. Exact-line cites, antipattern push, confidence grading — SHIPPED `fd8bc5b`
...
#### 9. `adr drift` — periodic full-repo scan vs principles — SHIPPED `9b3c2c3`
...
#### 10. GitLab + Bitbucket posters for `adr review` — SHIPPED `2039781`
...
#### 11. Batched CI review mode — SHIPPED `da909ab`
