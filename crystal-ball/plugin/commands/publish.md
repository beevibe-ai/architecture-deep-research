---
description: Publish the current Claude Code session as a Crystal Ball capsule — a queryable snapshot others can ask questions about. Use when the user says "share this session", "send this to my cofounder", "publish this as a crystal", or `/crystal publish`. The output is a share URL.
---
...
curl -fsS --max-time 2 "$CRYSTAL_SERVER/api/health" >/dev/null \
  || { echo "Crystal Ball server not reachable at $CRYSTAL_SERVER. Start it with 'npm run crystal:server' in the beevibe-cto repo, or set CRYSTAL_BALL_SERVER_URL to a hosted instance."; exit 1; }
```
